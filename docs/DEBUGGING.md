...
### 3.17 体素炮塔锚点三修：原点锚不得含 HVA 平移；地基中心非各格均值；比例独立
- **现象**：盖特机炮/巨炮 VXL 炮塔与底座错位（盖特尤其明显：炮管组件漂在
  底座上方；巨炮偶/奇行（cy 奇偶）错位程度还不一样，差 30px）。
- **根因（三个独立缺陷，MK 建成帧 F1 + 底座美术 + 照片对照定量）**：
  1. **原点锚被首节 HVA 平移污染**：`rasterize_voxel_parts` 的 origin 锚原用
     `secs[0].proj`（含该节 HVA 矩阵的平移）计算"模型原点"，而 HVA 平移只是
     "节几何相对模型原点的摆位"（YAGGUN 第 0 节是炮管 `DUMMY03`，t=(6.2,9.1,26.7)
     体素 → 抬到枢轴高度），不该进锚点。实测同一光栅 HVA 开/关 origin 差
     **(+15.4,−25.2)px@scale0.5**——整模随首节漂移；GTGCAN/BARL 平移≈0 故未暴露。
     修复 = 用单位 HVA 重建节投影再投影模型原点；载具 `anchor_*` 语义未动。
  2. **基准点用"地基各格顶顶点均值"**：2×2 在砖墙格栅下随行奇偶在 (+45,+7.5)/(+15,+7.5)
     间跳变 **30px**，而底座/MK 美术与奇偶无关。正确基准 = 地基几何中心（地图空间
     fw×fh 矩形中心的屏幕投影）：**锚点 + ((fw−fh)·15, (fw+fh)·7.5)**（2×2 → (0,30)、
     1×1 → (0,15)，与奇偶无关）。修复后 GTGCAN F1 0.02/0.37（偶/奇行）→ **0.713**。
  3. **比例随载具滑杆**（默认 0.3 → 0.6px/体素）：偏小且无法用偏移补偿。原版权威
     换算（ModEnc Voxel：**RA2 一格 = 42.4264 体素**，1 体素 = 6.03397 勒普顿）→
     每体素沿格轴 60/42.4264 = 1.4142px；本投影 `ex=(2s,−s)` → **s = 30/42.4264 =
     0.35355**。注意勿用 0.5（放大 41%：GTGCAN 炮塔罩 105px 占满 113px 平台，
     原版约占 2/3；用户实测"炮台太大"即此）。手绘 MK 的 F1 联合扫描峰值 0.36~0.44
     （NASAM 0.42/GTGCAN 0.44/YAGGUN 0.40/NAFLAK 0.36）也围绕 0.354，可交叉印证；
     YAGGUN 手绘宽度 43px 对模型 @0.354=45px（若按 det=1/18 缩到 0.236 则仅 30px，
     与画不符 → 渲染不额外乘 det，det 只用于 HVA 平移换算）。
- **手法**：① 判锚点是否污染看"同光栅 HVA 开/关 origin 差"；② 判基准点看
  "偶/奇行 F1 是否差 30px"；③ 判单位看 MK F1 扫描（GTGCAN 勒普顿+地基中心 0.713
  vs 像素 1:1 0.481 → 勒普顿定案）；④ 手调 nudge 全部删除（YAGGUN nudge 实为
  补偿根因 1，删后 F1 反而升）。
- **教训**：① 锚点必须从"姿态无关"的模型坐标系取（HVA 只是姿态）；② 手绘 MK 只作
  粗标定（±5px、炮管仰角会带偏 F1），但"30px 级奇偶跳变"这类结构性错误一测即现。
