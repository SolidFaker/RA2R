...
- **格占用（原版）**：一格 = **1 载具** 或 **最多 3 个步兵**（子格 0..2，
  ...
  - 行进门禁：段边界目标格已满 → 撤回本帧增量停在
    格内等位（不越界入占格）；**被挡立即重算（首帧）、仍被挡每 5 帧再试**（动态障碍：车辆/人物）→
    `replan_around_units()` 把占位单位当临时障碍从**当前格**绕行重规划
    （避免报错地从被占的段终点续接而死锁）；重算时把单位已走的**屏幕位移
    投影到新段方向**作为新 frac、保留 prev（来向，交给渲染端转角平滑）——
    单位**不弹回格心、不闪现**（方向接近时位置几乎不变）。
...
### 6.18 单位报错不能只停在“等位”——重规划要从当前格起算
同一格最多 3 步兵/1 载具：行进到达前要先看目标格是否有位（否则
撤回本帧增量停在格内）；被堵住首帧立即把占位单位当障碍（仍被挡每 5 帧再试），且重算保留已走进度（frac 投影）与来向（不闪现）
重规划——重规划必须从**当前格**起算（段终点已被占，从它续接会
永远等在边界上）。多单位集中目标时用“目标槽位”分散，避免扎堆。
