...
- **血条（参考图）**：
  ...
  - 立体感：顶面 2px（亮）、正面 5px（本色）、底面 2px（暗）、两端封口 +
    ...
    4×4 → 88）。
- **三棱线（仅选中建筑）**：地基的北（锚格顶顶点）/东（最右格右顶点）/
  西（最左格左顶点）三个角各画三条棱：+x/+y（一个格边，屏幕
  (±30,15)）与 +z（格子垂直的一半 = 15px）。**北角必须用锚格**
  （地图空间顶格）而不是“扫最小 y 的格”——同一行多格的 bbox y
  相同，扫描会挑错格（检查图片时发现北棱不在锚格上）。
