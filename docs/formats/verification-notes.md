...
## 6. M3 追加实测（SHP 阴影/帧段、HVA、VXL 调色板、NewTheater）

> 详细规格已并入 `formats/shp.md` §5.1/§5.3/§5.4、`formats/hva.md` §3/§3.1、
> `formats/vxl.md` §3.3；这里只列"推翻了什么旧结论"。

- **勘误：SHP 阴影索引是 1，不是 4**。全库 1200 个 SHP / 6363 个单色帧中 6292 个是索引 1；
  渲染时重映射为 ARGB(140,0,0,0)（OpenRA `PaletteFromFile ShadowIndex=1` 同义）。
  取原色会得到深蓝影子（`UNITURB.PAL` 索引 1 = (0,0,49)）。
- **勘误：建筑/动画 SHP 的帧段顺序是 `[空闲 L][受损 L][阴影 L][阴影受损 L]`**，
  不是 `[动画][阴影][受损][受损阴影]`；阴影段恒在后半（起点 n/2）；受损变体在**第 2 段**。
  `ActiveAnimDamaged=<名>_AD` 指向的文件在原版数据中**不存在**。
- **勘误：HVA 矩阵在文件里是列主序**（12 float，每 4 个一列），且**平移单位 = 1/16 体素**
  （`YTNKTUR` tz=256.7 → 16.0，恰为 15 体素高车体的炮塔高度）。
- **勘误：VXL 内嵌调色板是 8-bit（0–255）**，`.PAL` 才是 6-bit（0–63）；体素再 `<<2` 会过曝。
- **新发现：NewTheater 剧场代号** = SHP 名第 2 字母：T=温和、A=雪、U=城市、D=沙漠、L=月球、
  N=新城市、G=通用回退。类型名（GAPOWR/YAGGUN…）第 2 字母恰为 **A（雪）**，
  按原名加载会拿到雪地美术；巨炮底座原名 `GTGCAN.SHP` 不存在（应为 `GGGCAN.SHP`）。
- **新发现：建筑精灵的阴影帧起点 = 帧数 n/2**（6 帧 → 3、8 帧 → 4）；
  "帧 0/1/2 各 +3" 只对 6 帧本体成立（8 帧本体的第 4 帧是建造脚手架彩色帧）。
- **新发现：体素炮塔锚点 = 主体节包围盒中心（x,y）+ 模型地面 z**；模型原点常偏在主体一侧
  （`YAGGUN` 主体中心在模型 x=+12）。
