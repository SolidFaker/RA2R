...
- **索引 0 = 透明**：渲染时跳过。
- **索引 4 = 阴影色**：RA2/TS 常用调色板索引 4 作为阴影（深灰）色（XCC 的 `shp_split_shadows`/`combine_shadows` 显式把索引 4 识别为阴影）。
...
RA2/TS SHP 中影子有两种常见组织方式，均非强制格式要求：

1. **内联阴影**：阴影直接画进每帧，用调色板索引 4（深色）表示；XCC 的 `combine_shadows`/`shp_split_shadows` 即按「索引 4 = 阴影」拆分/合成。
2. **独立阴影帧**：阴影作为独立帧存放，常见为「前半 `N/2` 帧 = 正常，后半 `N/2` 帧 = 阴影」，阴影帧中非透明像素统一映射为阴影色（XCC `extract_as_pcx(combine_shadows=true)` 正是按 `i == c_images/2` 为分界把后半段当阴影处理）。

> 具体以 `art(md).ini` 的 `Shadow` / 序列定义为准，本惯例属经验性描述，**待验证**细节。
...
5. **像素语义**：索引 0 = 透明，索引 4 = 阴影，索引 16–31 = 阵营/玩家色重映射区（ModEnc 明确）；影子既可是内联索引 4，也可是后半段独立影子帧（经验惯例，细节待验证）。
